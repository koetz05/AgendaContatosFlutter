...
final String telColumn = "telColumn";
...
class ContactHelper {
    ...
    return await openDatabase(
      ...
      await db.execute(
          "CREATE TABLE $contactTable($idColumn INTEGER PRIMARY KEY,"
              "$nameColumn TEXT, $emailColumn TEXT, $telColumn TEXT, "
              "$imgColumn TEXT)"
      );
    });
  }
  ...
  Future<Contact> getContact(int idContact) async {
    ...
    List<Map> maps = await dbContact.query(contactTable,
        columns: [idColumn, nameColumn, telColumn, emailColumn, imgColumn],
        where: "$idColumn = ?",
        whereArgs: [idContact]);
  ...
  Future<List> getAllContact() async {
    ...
    for (Map m in maps) {
      contatos.add(Contact.fromMap(m));
    }
    return contatos;
  }
  ...
  Future<int> deleteContact(int idContact) async {
    ...
    return await dbContact.delete(
        contactTable, where: "$idColumn = ?", whereArgs: [idContact]);
  }
...
class Contact {
  ...
  @override
  String toString() {
    return "Contact(id: $id. name: $name, email: $email, tel: $tel, img: $img)";
  }
